class cappetta-datastax::refresh_cluster{
  ...
  exec { 'refresh cassandra config file: topology.properties':
    command => 'cp /vagrant/puppet/modules/cappetta-datastax/files/cassandra/cassandra-topology.properties /etc/cassandra',
    refreshonly => $refresh,
  }->

  exec { 'refresh cassandra config file: cassndra.yaml':
    command => 'cp /vagrant/puppet/modules/cappetta-datastax/files/cassandra/cassandra.yaml /etc/cassandra',
    refreshonly => $refresh,
  }->

  exec { 'refresh cassandra config file: custom_metrics.yaml':
    command => 'cp /vagrant/puppet/modules/cappetta-datastax/files/refresh_cluster.pp/jmxtrans.cassandra.yaml /etc/cassandra',
    refreshonly => $refresh,
  }->

  exec { 'refresh cassandra config file: env.sh':
    command => 'cp /vagrant/puppet/modules/cappetta-datastax/files/cassandra/cassandra-env.sh /etc/cassandra',
    refreshonly => $refresh,
  }->
